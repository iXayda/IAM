...
@ConfigurationProperties("iam.authorization.token-protection")
record AuthorizationTokenProtectionProperties(String activeKeyId, Map<String, String> keys) {

	private static final int KEY_BYTES = 32;

	AuthorizationTokenProtectionProperties {
		...
		if (activeKeyId == null && !keys.isEmpty()) {
			throw new IllegalArgumentException("An active authorization token protection key ID is required");
		}
		if (activeKeyId != null && !keys.containsKey(activeKeyId)) {
			throw new IllegalArgumentException("The active authorization token protection key is not configured");
		}
	}
	...
	public String toString() {
		return "AuthorizationTokenProtectionProperties[activeKeyId=" + this.activeKeyId
	...
	private static String requireKeyId(String value) {
		if (value == null || value.isEmpty() || value.length() > 64
			...
			throw new IllegalArgumentException(
					"Authorization token protection key IDs must contain 1 to 64 letters, digits, dots, underscores, or hyphens");
		}
		return value;
	}
	...
	private static void validateEncodedKey(String keyId, String encodedKey) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Authorization token protection key '" + keyId + "' must be valid Base64", exception);
		}
		try {
			if (decoded.length != KEY_BYTES) {
				throw new IllegalArgumentException(
						"Authorization token protection key '" + keyId + "' must contain exactly 32 bytes");
			}
		}
		finally {
