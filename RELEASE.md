...
- 匿名 SCIM 2.0 discovery、受 scope 保护的资源命名空间、租户隔离的 User 生命周期与 Group 读取和创建；
- 健康探针、Prometheus 指标、OpenTelemetry 和告警规则。

OAuth 2.0/OIDC、SCIM discovery、Users 生命周期和 Group 读取与创建已提供 HTTP 入口并纳入发布门禁。
SCIM Groups 更新与删除、Admin RBAC、完整 MFA 和审计 HTTP 入口尚未提供，
...
5. SCIM discovery、租户隔离的 User 生命周期与 Group 读取和创建、service JWT read/write 边界、OAuth 2.0 Authorization Code、PKCE、OIDC、Refresh Token 和 Client Credentials HTTP smoke；
...
- 已明确记录尚未提供的 SCIM Groups 更新与删除、Admin RBAC、完整 MFA 和审计 HTTP 能力。
