...
- 匿名 SCIM 2.0 discovery、受 scope 保护的资源命名空间，以及租户隔离的 User 创建、替换与读取；
- 健康探针、Prometheus 指标、OpenTelemetry 和告警规则。

OAuth 2.0/OIDC、SCIM discovery 和 Users 创建、完整替换、读取已提供 HTTP 入口并纳入发布门禁。
SCIM Users PATCH 与删除、Groups provisioning、Admin RBAC、完整 MFA 和审计 HTTP 入口尚未提供，
...
5. SCIM discovery、租户隔离的 User 创建、替换与读取、service JWT read/write 边界、OAuth 2.0 Authorization Code、PKCE、OIDC、Refresh Token 和 Client Credentials HTTP smoke；
...
- 已明确记录尚未提供的 SCIM Users PATCH 与删除、Groups provisioning、Admin RBAC、完整 MFA 和审计 HTTP 能力。
