...
- 每次生成 10 个一次性恢复码，仅保存自适应哈希并通过数据库条件更新原子消费
...
调用方必须重新登录。`POST /iam/account/mfa/recovery-codes` 原子替换恢复码并只返回一次 10 个明文值；
替换同样要求近期密码认证、CSRF token，并在成功后立即失效 session。MFA 状态响应同时包含
`totpEnabled` 和 `recoveryCodesAvailable`。
...
TOTP secret 使用独立的 AES-256-GCM key ring 加密，AAD 绑定 tenant、User、credential、保护版本和
TOTP 参数。生产环境必须配置 `IAM_TOTP_SECRET_ACTIVE_KEY_ID` 和对应的
